...
from mini_libact import QueryStrategy
...
class HierarchicalSampling(QueryStrategy):
    ...
    def __init__(self, dataset, classes, active_selecting=True,
        ...
        self.random_state_ = np.random.default_rng(seed=random_state)
    ...
    def _sample_node(self, node):
        ...
        if self.random_state_.random() < p_left:
    ...
    def _get_delta(self, frac, node):
        ...
        return fs_corr / self.total[node] + np.sqrt(fs_corr * frac * (1. - frac) / self.total[node])
