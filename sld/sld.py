...
def run_sld(y, posteriors_zero, priors_zero, epsilon=1e-6, positive_class=1):
    """
    ...
    history.append(History(s, list(priors_s), posteriors_zero, 1, acc, prec, rec))
    while not val < epsilon:
        ...
        s += 1
        history.append(History(s, list(priors_s), posteriors_s, val, acc, prec, rec))
